...
if(!empty($_GET['id'])){
    ...
    $userData = $db->getRows('archive_db', $conditions);
}
...
<div class="panel">
    ...
    <?php } ?>
    <div class="panel-heading heavier"><?php echo $actionLabel; ?> USER <a href="linkmultifolder.php"> <i class='bx bx-arrow-back'></i></a></div>
        <form method="post" action="userAction.php" class="form">
            ...
            <div class="form-group">
                <label>Type</label>
                <input type="text" class="form-control" name="type_name" value="<?php echo 'Link'; ?>" disabled required>
            </div>
            <input type="hidden" name="id" value="<?php echo !empty($userData['id'])?$userData['id']:''; ?>">
            <input type="submit" name="userSubmit" class="input_button bolder" value="SUBMIT"/>
        </form>
    </div>
